...
def RUN(filename1,):
    ...
    dfTOP_RFR56.to_csv(ROOT+f'Top_1000_{filename1}.csv')
    dfTEST_RFR56_grouped = dfTEST_RFR56.groupby('PCID').agg('mean')
    dfTEST_RFR56_grouped.to_csv(ROOT+f'TEST_{filename1}.csv')
    ...
    df_final.to_csv(ROOT+f'Top_100_Aggregated_{filename1}.csv')
